...
func (rf *ResultFormatter) Format(toolName string, params map[string]interface{}, result Result, duration time.Duration) string {
...
func (rf *ResultFormatter) formatByToolType(toolName string, params map[string]interface{}, result Result) string {
	switch toolName {
	case "datetime":
...
func (rf *ResultFormatter) formatCalculator(params map[string]interface{}, data interface{}) string {
	...
	if !exists {
		return fmt.Sprintf("计算工具执行了 %s 操作，但无法获取结果", operation)
	}

	return fmt.Sprintf("计算结果 (%s): %.2f", operation, resultValue)
}
