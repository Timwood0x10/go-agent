...
func NewTaskPlanner(llmClient *llm.Client) *TaskPlanner {
	...
	return &TaskPlanner{
		BaseTool:  NewBaseToolWithCategory("task_planner", "Plan and decompose tasks into executable steps", CategoryCore, params),
		llmClient: llmClient,
	}
}

// Execute performs the task planning operation.
func (t *TaskPlanner) Execute(ctx context.Context, params map[string]interface{}) (Result, error) {
	...
	if !ok || goal == "" {
		return NewErrorResult("goal is required"), nil
	}
	...
	case "decompose_task":
		...
		if !ok || task == "" {
			return NewErrorResult("task is required for decompose_task operation"), nil
		}
...
func (t *TaskPlanner) planTasks(ctx context.Context, goal, context string, availableTools []string) (Result, error) {
	...
	if t.llmClient == nil {
		return NewErrorResult("LLM client not available for task planning"), nil
	}

	response, err := t.llmClient.Generate(ctx, prompt)
	if err != nil {
		return NewErrorResult(fmt.Sprintf("failed to generate plan: %v", err)), nil
	}
...
func (t *TaskPlanner) decomposeTask(ctx context.Context, task, complexity string) (Result, error) {
	...
	if t.llmClient == nil {
		return NewErrorResult("LLM client not available for task decomposition"), nil
	}

	response, err := t.llmClient.Generate(ctx, prompt)
	if err != nil {
		return NewErrorResult(fmt.Sprintf("failed to decompose task: %v", err)), nil
	}
...
func (t *TaskPlanner) estimateTime(ctx context.Context, goal string) (Result, error) {
	...
	response, err := t.llmClient.Generate(ctx, prompt)
	if err != nil {
		return NewErrorResult(fmt.Sprintf("failed to estimate time: %v", err)), nil
	}
