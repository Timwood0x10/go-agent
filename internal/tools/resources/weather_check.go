...
type WeatherProvider interface {
	...
	GetForecast(ctx context.Context, location string, days int) ([]*WeatherData, error)
}
...
func NewWeatherCheck(provider WeatherProvider) *WeatherCheck {
	...
	wc.BaseTool = NewBaseTool("weather_check", "Check weather for a location", params)

	return wc
}

// Execute performs the weather check.
func (t *WeatherCheck) Execute(ctx context.Context, params map[string]interface{}) (Result, error) {
	location, ok := params["location"].(string)
	if !ok || location == "" {
		return NewErrorResult("location is required"), nil
	}
...
func (m *MockWeatherProvider) GetForecast(ctx context.Context, location string, days int) ([]*WeatherData, error) {
	result := make([]*WeatherData, days)
