...
func NewCodeRunner() *CodeRunner {
	...
	return &CodeRunner{
		BaseTool:      NewBaseToolWithCategory("code_runner", "Execute Python and JavaScript code with sandbox constraints", CategorySystem, params),
		enablePython:  true,
...
func (t *CodeRunner) Execute(ctx context.Context, params map[string]interface{}) (Result, error) {
	...
	if err := t.validateCode(code); err != nil {
		return NewErrorResult(fmt.Sprintf("code validation failed: %v", err)), nil
	}
	...
	case "run_python":
		if !t.enablePython {
			return NewErrorResult("Python execution is disabled"), nil
		}
		return t.runPython(execCtx, code, maxOutputSize)
	case "run_js":
		if !t.enableJS {
			return NewErrorResult("JavaScript execution is disabled"), nil
		}
...
func (t *CodeRunner) runPython(ctx context.Context, code string, maxOutputSize int) (Result, error) {
...
func (t *CodeRunner) runJavaScript(ctx context.Context, code string, maxOutputSize int) (Result, error) {
