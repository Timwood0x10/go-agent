...
func NewRegexTool() *RegexTool {
	...
	return &RegexTool{
		BaseTool: NewBaseToolWithCategory("regex_tool", "Perform regex match, extract, and replace operations", CategoryCore, params),
	}
}

// Execute performs the regex operation.
func (t *RegexTool) Execute(ctx context.Context, params map[string]interface{}) (Result, error) {
	...
	if err != nil {
		return NewErrorResult(fmt.Sprintf("invalid regex pattern: %v", err)), nil
	}
	...
	case "replace":
		...
		if !ok || replacement == "" {
			return NewErrorResult("replacement is required for replace operation"), nil
		}
...
func (t *RegexTool) match(ctx context.Context, text string, re *regexp.Regexp, maxResults int) (Result, error) {
...
func (t *RegexTool) extract(ctx context.Context, text string, re *regexp.Regexp, maxResults int) (Result, error) {
...
func (t *RegexTool) replace(ctx context.Context, text string, re *regexp.Regexp, replacement string) (Result, error) {
