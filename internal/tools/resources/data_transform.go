...
func NewDataTransform() *DataTransform {
	...
	return &DataTransform{
		BaseTool: NewBaseToolWithCategory("data_transform", "Transform data between CSV, JSON, and flattened formats", CategoryData, params),
	}
}

// Execute performs the data transformation operation.
func (t *DataTransform) Execute(ctx context.Context, params map[string]interface{}) (Result, error) {
...
func (t *DataTransform) csvToJSON(ctx context.Context, csvData, delimiter string, hasHeader bool) (Result, error) {
	reader := csv.NewReader(strings.NewReader(csvData))
	reader.Comma = []rune(delimiter)[0]

	records, err := reader.ReadAll()
	if err != nil {
		return NewErrorResult(fmt.Sprintf("failed to parse CSV: %v", err)), nil
	}
	...
	if hasHeader {
		if len(records) < 1 {
			return NewErrorResult("CSV is empty"), nil
		}
...
func (t *DataTransform) jsonToCSV(ctx context.Context, jsonData, delimiter string) (Result, error) {
	var js interface{}
	...
	if !ok {
		return NewErrorResult("JSON must be an array of objects"), nil
	}
...
func (t *DataTransform) flattenJSON(ctx context.Context, jsonData, separator string) (Result, error) {
	var js interface{}
	if err := json.Unmarshal([]byte(jsonData), &js); err != nil {
		return NewErrorResult(fmt.Sprintf("invalid JSON: %v", err)), nil
	}
